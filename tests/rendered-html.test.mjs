...
test("keeps upstream access fixed and the production listener private", async () => {
  ...
  assert.match(satelliteSnapshots, /BURNT_REFRESH_INTERVAL_MS = 6 \* 60 \* 60 \* 1000/);
  ...
  assert.match(dashboard, /width < 700 \? 12 : 24/);
  assert.match(dashboard, /timestamp - previousFrame < 50/);
